...
class TestGrader:
    ...
    def generate_mask(self):
        # Create a mask to focus only on the question region
        mask = np.zeros(self.gray.shape, dtype="uint8")
        mask[int(0.15*self.image.shape[0]):int(0.85*self.image.shape[0]), :] = 255
        return mask

    def grade_test(self):
        # Apply the mask to the grayscale image
        masked_gray = cv2.bitwise_and(self.gray, self.mask)
...
# Используйте этот код для создания объекта класса и обработки изображения:
